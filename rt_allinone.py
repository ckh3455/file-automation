...
def _is_month_first_today_single_day_range(req_start: date, req_end: date) -> bool:
    ...
    return (
        tk.day == 1
        and req_start == req_end == tk
        and req_start == month_first(tk)
    )
...
def fetch_and_process(
    ...
    for attempt in range(1, CLICK_RETRY_MAX + 1):
        ...
        try:
            ...
            got_file = got
            debug(
                f"  - got file: {got_file}  size={got_file.stat().st_size:,}  ext={got_file.suffix}"
            )
            break
        except TimeoutError:
            debug(
                f"  - warn: 다운로드 시작 감지 실패(시도 {attempt}/{CLICK_RETRY_MAX})"
            )
            if attempt % 5 == 0:
                driver.switch_to.default_content()
                driver.get(URL)
    ...
    if not got_file:
        ...
        if _is_month_first_today_single_day_range(start, end):
            ...
            drive_upload_and_cleanup(creds, out)  # ARTIFACTS_ONLY=1이면 내부에서 스킵
            return
...
def main():
    ...
    try:
        ...
        # 저장 결과 요약
        debug("== saved xlsx in output ==")
        files = sorted(SAVE_DIR.glob("*.xlsx"))
        if not files:
            debug("  (none)")
        else:
            for p in files:
